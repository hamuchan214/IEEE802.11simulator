...
def simulate_transmission(users, duration):
    ...
    while current_time < duration:
        ...
        if collisions:
            collision_count += 1
            current_time += min_user.CW
            
            print(f"\nTime: {current_time:.2f}s - Collision detected! Users: {[user.id for user in collisions] + [min_user_id]}")
            
            current_time += transmission_time(data_transmission, transmission_rate)
            ...
            if current_time + trans_time <= duration:
                current_time += min_cw
                print(f"\nTime: {current_time:.2f}s - User {min_user_id} transmitted successfully with CW = {min_cw:.6f} seconds (waited {min_user.slots} slots)")
